...
- 为保护店铺隐私，SKU、ASIN、广告活动、搜索词可能已经用 SKU-001、ASIN-001、CAMPAIGN-001、TERM-001 等编号脱敏。请直接引用这些编号，不要猜测真实名称。
...
class PrivacyMapper:
    ...
    def mask(self, value: Any, prefix: str) -> str:
        ...
        if key not in self.forward:
            ...
            alias = f"{prefix}-{self.counters[prefix]:03d}"
            self.forward[key] = alias
            self.reverse[alias] = text
        return self.forward[key]


def _restore_private_refs(value: Any, reverse_map: dict[str, str]) -> Any:
    ...
    if isinstance(value, list):
        return [_restore_private_refs(item, reverse_map) for item in value]
    if isinstance(value, dict):
        return {key: _restore_private_refs(item, reverse_map) for key, item in value.items()}
    return value
...
def build_private_operations_context(context: dict[str, Any]) -> tuple[dict[str, Any], dict[str, str]]:
    mapper = PrivacyMapper()
    private_context = json.loads(json.dumps(context, ensure_ascii=False, default=str))

    for row in private_context.get("top_skus_by_profit", []):
        row["sku"] = mapper.mask(row.get("sku"), "SKU")
        row["asin"] = mapper.mask(row.get("asin"), "ASIN")
        if row.get("title"):
            row["title"] = f"脱敏产品标题-{row['sku'] or row['asin']}"

    for row in private_context.get("weak_skus_by_profit", []):
        row["sku"] = mapper.mask(row.get("sku"), "SKU")
        row["asin"] = mapper.mask(row.get("asin"), "ASIN")
        if row.get("title"):
            row["title"] = f"脱敏产品标题-{row['sku'] or row['asin']}"

    for row in private_context.get("rule_ad_actions", []):
        row["search_term"] = mapper.mask(row.get("search_term"), "TERM")
        row["campaign"] = mapper.mask(row.get("campaign"), "CAMPAIGN")

    for row in private_context.get("listing_audits", []):
        row["sku"] = mapper.mask(row.get("sku"), "SKU")
        row["asin"] = mapper.mask(row.get("asin"), "ASIN")
        if row.get("title"):
            row["title"] = f"脱敏产品标题-{row['sku'] or row['asin']}"

    private_context["privacy_notice"] = (
        "Identifiers sent to the LLM are anonymized. Local system restores aliases after the response."
    )
    return private_context, mapper.reverse
...
def generate_operations_ai_report(db: Session) -> OperationsAiReport:
    ...
    private_context, reverse_map = build_private_operations_context(context)
    prompt = OPERATIONS_USER_PROMPT.format(context_json=json.dumps(private_context, ensure_ascii=False, indent=2, default=str))
    try:
        ...
        data = _restore_private_refs(data, reverse_map)
