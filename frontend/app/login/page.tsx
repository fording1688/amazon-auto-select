...
export default function LoginPage() {
  ...
  return (
    <main className="mx-auto max-w-md px-6 py-12">
      ...
      <p className="mt-2 text-slate-600">登录后只能看到你自己的店铺数据、报表批次和 Listing 项目。</p>
      ...
      <p className="mt-4 text-sm text-slate-600">还没有账号？<Link className="font-semibold text-blue-700" href="/register">去注册</Link></p>
    </main>
  );
}
...
function Input({ name, label, type }: { name: string; label: string; type: string }) {
  return (
    <label className="block text-sm font-semibold text-slate-700">
      {label}
      <input className="mt-2 w-full rounded-md border border-slate-300 px-3 py-2" name={name} type={type} required />
    </label>
  );
}
