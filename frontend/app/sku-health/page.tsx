  ...
  return (
    <main className="mx-auto max-w-7xl px-6 py-8">
      ...
      <div className="mt-6 overflow-x-auto rounded-lg border bg-white p-4">
        <table className="w-full text-left text-sm"><thead><tr className="border-b"><th>SKU</th><th>销售</th><th>利润</th><th>库存天数</th><th>健康分</th><th>标签</th><th>建议</th></tr></thead><tbody>
          {items.map((item) => <tr className="border-b" key={`${item.sku}-${item.asin}`}><td className="py-3 font-medium">{item.sku}<br/><span className="text-slate-500">{item.asin}</span></td><td>${item.sales?.toFixed?.(2) ?? item.sales}</td><td>${item.estimated_profit?.toFixed?.(2) ?? item.estimated_profit}</td><td>{item.inventory_days ?? '-'}</td><td>{item.health_score}</td><td>{item.problem_tags?.join(', ')}</td><td>{item.recommended_actions?.[0]}</td></tr>)}
        </tbody></table>
